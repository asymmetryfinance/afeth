...
abstract contract VotiumStrategyCore is IVotiumStrategy, OwnableUpgradeable, ERC20Upgradeable {
    ...
    event DepositReward(uint256 indexed newPrice, uint256 indexed ethAmount, uint256 indexed cvxAmount);
    ...
    function cvxInSystem() public view returns (uint256) {
        uint256 total = ILockedCvx(VLCVX_ADDRESS).lockedBalanceOf(address(this));
    ...
    function ethPerCvx(bool _validate) public view returns (uint256) {
        ...
        if (
            (
                !_validate
                    || (
                        cl.success == true && cl.roundId != 0 && cl.answer > 0 && cl.updatedAt != 0
                            && cl.updatedAt <= block.timestamp && block.timestamp - cl.updatedAt <= 25 hours
                    )
            )
        ) {
     ...
     */
    function claimRewards(IVotiumMerkleStash.ClaimParam[] calldata _claimProofs) external onlyRewarder {
     ...
     */
    function applyRewards(SwapData[] calldata _swapsData, uint256 _safEthMinout, uint256 _cvxMinout, uint256 _deadline)
        external
        onlyRewarder
    {
        ...
        for (uint256 i = 0; i < _swapsData.length; i++) {
            ...
            uint256 allowance = IERC20(_swapsData[i].sellToken).allowance(address(this), address(_swapsData[i].spender));
            if (allowance != type(uint256).max) {
                if (allowance > 0) {
                    IERC20(_swapsData[i].sellToken).safeApprove(address(_swapsData[i].spender), 0);
                }
                IERC20(_swapsData[i].sellToken).safeApprove(address(_swapsData[i].spender), type(uint256).max);
            }
            (bool success,) = _swapsData[i].swapTarget.call(_swapsData[i].swapCallData);
            if (!success) {
     ...
     * @param _claimProofs - Array of claim proofs
     */
    function claimVotiumRewards(IVotiumMerkleStash.ClaimParam[] calldata _claimProofs) private {
        IVotiumMerkleStash(0x378Ba9B73309bE80BF4C2c027aAD799766a7ED5A).claimMulti(address(this), _claimProofs);
    }
