...
abstract contract NftStrategy is Initializable, OwnableUpgradeable, ERC1155Upgradeable {
    ...
    function mint() virtual external payable returns (uint256);

    /// request to close a position
    function requestClose(uint256 positionId) virtual external;

    /// check if a position has fully closed and can be burned
    function burnable(uint256 positionId) virtual external view returns (uint256);
    ...
    function claimable(uint256 positionId) virtual external view returns (uint256);

    /// how much has already been claimed from a position
    function claimed(uint256 positionId) virtual external view returns (bool);

    /// current value of a position if it were to be burned right now
    function currentValue(uint256 positionId) virtual external view returns (uint256);
}
