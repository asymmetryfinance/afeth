...
describe("Test AfEth", async function () {
  ...
  const setCvxAmmPrice = async (newPrice: BigNumber) => {
    ...
    const ethSellAmount = ethers.utils.parseEther("1");
    const cvxSellAmount = ethers.utils.parseEther("1000");
  ...
  it.only("Should show that a 2% cvx price difference with a 90% ratio difference is only 1% difference in afEth received", async function () {
    ...
    const startingRatio = ethers.utils.parseEther("0.95");
    const differentRatio = ethers.utils.parseEther("0.05");
  ...
  it("Two users should be able to simultaneously deposit the same amount, requestWithdraw, withdraw and split rewards", async function () {
    ...
    const tx = await afEth.depositRewards(depositAmount, 0, 0, {
      value: depositAmount,
    });
    await tx.wait();
  ...
  it("Two users should be able to deposit at different times and split rewards appropriately", async function () {
    ...
    let tx = await afEth.depositRewards(depositAmount, 0, 0, {
      value: rewardAmount,
    });
    await tx.wait();
    ...
    tx = await afEth.depositRewards(depositAmount, 0, 0, {
      value: rewardAmount,
    });
    await tx.wait();
  ...
  it("Should be able to deposit rewards to all strategies", async function () {
    ...
    let tx = await afEth.depositRewards(rewardAmount, 0, 0, {
      value: rewardAmount,
    });
    await tx.wait();
    ...
    tx = await afEth.depositRewards(rewardAmount, 0, 0, {
      value: rewardAmount,
    });
    await tx.wait();
  ...
  it("Should show rewards push the ratio towards the target ratio", async function () {
    ...
    for (let i = 0; i < 20; i++) {
      const tx = await afEth.depositRewards(rewardAmount, 0, 0, {
        value: rewardAmount,
      });
      await tx.wait();
  ...
  it("Should fail if depositing rewards results in less safEth being created than _safEthMinout", async function () {
    const rewardAmount = ethers.utils.parseEther("1");

    expect(
      afEth.depositRewards(rewardAmount, "99999999999999999999999", 0, {
        value: rewardAmount,
      })
    ).to.be.revertedWith("BelowMinOut()");
  });
  it("Should fail if depositing rewards results in less safEth being created than _cvxMinout", async function () {
    ...
    expect(
      afEth.depositRewards(rewardAmount, 0, "99999999999999999999999", {
        value: rewardAmount,
      })
    ).to.be.revertedWith("BelowMinOut()");
  });
