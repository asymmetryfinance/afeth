...
export const updateRewardsMerkleRoot = async (
  ...
  const testData = await readJSONFromFile(
    "./scripts/testData.json"
  );

  const tokenAddresses = Object.keys(testData.claimProofs);

  // set root from new mocked merkle data
  for (let i = 0; i < tokenAddresses.length; i++) {
    const merkleRoot = testData.claimProofs[tokenAddresses[i]].merkleRoot;
  ...
  const claimProofs = tokenAddresses.map((_: any, i: number) => {
    const pd = testData.claimProofs[tokenAddresses[i]];
    return [
      tokenAddresses[i],
  ...
  return { claimProofs, swapsData: testData.swapsData };
};
